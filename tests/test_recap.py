...
def matches() -> list[Match]:
    return [make_match(q, s, e) for q, s, e in SERIES_QUOTES]
...
def test_segments_are_chronological_and_fill_the_target(
    ...
    plan = plan_recap(matches, 120.0, config, target_duration=20.0)
...
def test_too_few_moments_is_refused_with_a_reason(config: Config) -> None:
    plan = plan_recap(
        [make_match("one", 10, 12), make_match("two", 60, 62)],
        120.0, config, target_duration=45.0,
    )
...
def test_target_too_short_for_the_minimum_segment(
    ...
    plan = plan_recap(matches, 120.0, config, target_duration=10.0)
